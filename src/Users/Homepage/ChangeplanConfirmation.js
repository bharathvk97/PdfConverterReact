...
function ChangeplanConfirmation() {
  ...
  const baseplanToStandardPlan = () => {
    ...
    if (
      ...
      if (access_token !== '') {
        dispatch(
          baseplanmonthlytostandardplanmonthly((res) => {
            console.log("dispatch response", res);
            if (res.status === 200) {
              navigate("/baseplanmonthtostandardplanmonthsuccess");
            }
          })
        );
      }
  ...
  const baseplanToProplan = () => {
    if (
      ...
      if(access_token !== ''){
        dispatch(
          baseplanmonthlytoproplanmonthly((res) => {
            console.log('proplan',res);
            if(res.status === 200) {
              navigate("/baseplanmonthtoproplanmonthsuccess")
            }
          })
        )
      }
        ...
        <div className="main">
          ...
          >
            ...
            <div className="planchange-btn-div">
              <button
                id="changeplan-btn"
                onClick={baseplanToStandardPlan}
